...
\title{\heiti RoboTest：面向九宫格人机验证的多模态数据分析系统\\
\large 多模态数据分析课程实验报告}
\author{
小组3 \quad 多模态数据分析\\[0.5em]
刘瀚哲 \quad 22336148\\
王宇和 \quad 22355082\\
冯乙山 \quad 22336070
}
...
Click-all 模型将九宫格定位从单标签分类扩展为多标签 cell selection。图像侧仍按九个 cell 提取视觉特征，文本侧编码“请点击所有某类别”的中文指令，融合后对每个 cell 输出一个独立概率。推理时，模型根据阈值选择所有目标格；如果没有 cell 超过阈值，则回退到 top-1，避免输出空动作。被选中的 cell 按从小到大的格子编号转换为动作序列，形成可执行的 \code{move\_to\_cell/click/done} 输出。
...
\begin{longtable}{p{0.28\linewidth}p{0.64\linewidth}}
  ...
  \pathcode{src/multimodal_captcha/model.py} & 定义 \code{MultimodalGridLocator} 和 \code{ActionSequenceLocator}，实现 CNN、GRU、Transformer 和图文融合结构。\\
...
\begin{itemize}[leftmargin=2em]
  \item 补充训练曲线图、更多失败案例图片和弱类 contact sheet；
  \item 将早期 single-target 结果也迁移到严格 source-level split 协议下复测；
  \item 若需要提交 PDF，使用 XeLaTeX 编译本文档，并检查所有图片路径。
\end{itemize}
